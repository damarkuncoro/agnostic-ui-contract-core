...
exports.SchemaContractValidator = void 0;
...
class SchemaContractValidator {
     ...
     */
    validateRequiredCombinations(contract, errors, warnings) {
        // Core contracts must have specific variants
        if (contract.category === Contract_1.ContractCategory.CORE) {
        ...
        if (contract.category === Contract_1.ContractCategory.COMPONENT) {
            const hasInteractiveVariants = contract.variants.some(v => v.type === 'intent' && v.values.includes('primary'));
        ...
        if (contract.category === Contract_1.ContractCategory.THEME) {
     ...
     */
    validateCoreContractRules(contract, errors, warnings) {
     ...
     */
    validateComponentContractRules(contract, errors, warnings) {
     ...
     */
    validateThemeContractRules(contract, _errors, warnings) {
        // Theme contracts should focus on visual variants
        const visualVariants = ['color', 'tone', 'emphasis'];
        const hasVisualVariant = contract.variants.some(v => visualVariants.some(vv => v.name.includes(vv)));
        if (!hasVisualVariant) {
     ...
     */
    validateSkinContractRules(contract, _errors, warnings) {
        // Skin contracts should have styling-related props
        const styleProps = ['className', 'style'];
        const hasStyleProp = contract.props.some(p => styleProps.includes(p.name));
        if (!hasStyleProp) {
     ...
     */
    validateUtilityContractRules(contract, _errors, warnings) {
     ...
     */
    validateAccessibilityRequirements(contract, errors, warnings) {
        if (!contract.hasAccessibility) {
            // Check if accessibility should be required
            const hasInteractiveProps = contract.props.some(p => ['onClick', 'onChange', 'onSubmit'].some(event => p.name.includes(event)));
        ...
        if (accessibility.keyboardActions.length > 0) {
            ...
            const invalidActions = accessibility.keyboardActions.filter(action => !validActions.includes(action));
     ...
     */
    validateVariantCompatibility(contract, errors, _warnings) {
     ...
     */
    validatePropConsistency(contract, errors, warnings) {
     ...
     * Validates metadata
     */
    validateMetadata(contract, errors, warnings) {
...
exports.SchemaContractValidator = SchemaContractValidator;
//# sourceMappingURL=SchemaContractValidator.js.map