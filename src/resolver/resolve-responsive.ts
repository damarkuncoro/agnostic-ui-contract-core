...
export function resolveResponsive(
  ...
  const responsive = (value as { $responsive: Record<string, unknown> }).$responsive
 ...
 */
function isValidResponsiveValue(value: unknown): value is { $responsive: Record<string, unknown> } {
  return (
    typeof value === "object" &&
    value !== null &&
    "$responsive" in value &&
    typeof (value as any).$responsive === "object" &&
    (value as any).$responsive !== null
  )
}
 ...
 */
function getSortedBreakpoints(breakpoints: string[]): string[] {
 ...
 */
function getBreakpointIndex(sortedBreakpoints: string[], targetBreakpoint: string): number {
  const semanticOrder = ['xs', 'sm', 'md', 'lg', 'xl', '2xl', '3xl', '4xl']
